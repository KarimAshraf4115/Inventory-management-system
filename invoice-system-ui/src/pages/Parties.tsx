...
export function PartiesPage({ kind, onNavigate }: Props) {
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ name: '', phone: '', balance: 0, type: isCustomers ? undefined : 'مصنع' });
    setOpen(true);
  };
  ...
  return (
    <div>
      ...
      <DataTable<Party>
        columns={[
          { key: 'name', header: 'الاسم', render: (p) => <span className="font-kufi font-semibold text-ink">{p.name}</span> },
          { key: 'phone', header: 'الهاتف', render: (p) => <span className="font-mono text-gray-600" dir="ltr">{p.phone}</span> },
          ...(isCustomers
            ? []
            : [{
                key: 'type',
                header: 'النوع',
                render: (p: Party) => (
                  <Badge className="bg-navy-50 text-navy-700 ring-navy-200">{p.type ?? '—'}</Badge>
                ),
              }]),
          {
            key: 'balance',
            header: 'الرصيد',
            render: (p) => {
              const debt = p.balance < 0;
              const credit = p.balance > 0;
              return (
                <span
                  className={`font-kufi font-bold ${
                    debt ? 'text-red-600' : credit ? 'text-emerald-600' : 'text-gray-500'
                  }`}
                >
                  {debt ? 'له' : credit ? 'عليه' : '—'} {p.balance !== 0 ? formatEGP(Math.abs(p.balance)) : ''}
                </span>
              );
            },
          },
      ...
      <Modal
        ...
        title={editing ? (isCustomers ? 'تعديل عميل' : 'تعديل مورد') : isCustomers ? 'إضافة عميل' : 'إضافة مورد'}
        footer={
          <>
        ...
        <div className="space-y-4">
          <Field label="الاسم">
            <input
              ...
              placeholder={isCustomers ? 'اسم العميل / المحل' : 'اسم المورد / المصنع'}
            />
          </Field>
