...
export function PaymentsPage() {
  ...
  const matchedInvoices = useMemo(() => {
    ...
    return invoices.filter((inv) => {
      const party =
        (inv.type === 'sale' ? customers : suppliers).find((p) => p.id === inv.partyId)?.name ?? '';
  ...
  const selectedParty = selected
    ? (selected.type === 'sale' ? customers : suppliers).find((p) => p.id === selected.partyId)
    : null;
        ...
        {query && (
          <div className="mt-2 max-h-48 overflow-y-auto rounded-lg border border-line">
            {matchedInvoices.length === 0 ? (
              <p className="p-3 text-center text-xs text-gray-400">لا توجد نتائج</p>
            ) : (
              matchedInvoices.map((inv) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Badge className={inv.type === 'sale' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-navy-50 text-navy-700 ring-navy-200'}>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
            ...
            <div className="rounded-xl2 border border-line bg-white shadow-card">
              <div className="border-b border-line px-4 py-3">
                ...
                <p className="mt-0.5 text-xs text-gray-400">
                  {selectedParty?.name} • {selected.type === 'sale' ? 'بيع' : 'شراء'}
                </p>
              </div>
              {invPayments.length === 0 ? (
                <p className="p-8 text-center text-sm text-gray-400">لا توجد دفعات مسجلة بعد</p>
              ) : (
                <DataTable<Payment>
                  columns={[
                    { key: 'date', header: 'التاريخ', render: (p) => <span className="text-gray-500">{formatDate(p.date)}</span> },
                    { key: 'amount', header: 'المبلغ', render: (p) => <span className="font-kufi font-bold text-emerald-600">{formatEGP(p.amount)}</span> },
                    { key: 'method', header: 'طريقة الدفع', render: (p) => <Badge className="bg-navy-50 text-navy-700 ring-navy-200">{methodLabel[p.method]}</Badge> },
                    { key: 'notes', header: 'ملاحظات', render: (p) => <span className="text-gray-500">{p.notes ?? '—'}</span> },
                  ]}
                  rows={invPayments}
                />
              )}
              ...
              {selected.initialPaid > 0 && (
                <div className="flex items-center justify-between border-t border-line bg-canvas/50 px-4 py-3 text-sm">
                  <span className="font-kufi text-gray-500">دفعة مقدم (عند إنشاء الفاتورة)</span>
                  <span className="font-kufi font-bold text-emerald-600">{formatEGP(selected.initialPaid)}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            <div className="rounded-xl2 border border-line bg-white p-5 shadow-card">
              <h3 className="mb-3 font-kufi text-sm font-bold text-ink">ملخص الفاتورة</h3>
              <div className="space-y-2.5 text-sm">
                <Row label="الإجمالي" value={formatEGP(invoiceTotal(selected))} />
                <Row label="المدفوع" value={formatEGP(invoicePaid(selected, payments))} valueClass="text-emerald-600" />
                <div className="flex items-center justify-between border-t border-line pt-2.5">
                  <span className="font-kufi font-semibold text-gray-500">المتبقي</span>
            ...
            <div className="rounded-xl2 border border-line bg-white p-5 shadow-card">
              <h3 className="mb-3 flex items-center gap-2 font-kufi text-sm font-bold text-ink">
                <PlusCircle size={16} className="text-navy-600" /> تسجيل دفعة جديدة
              </h3>
              <div className="space-y-3">
                ...
                <Button className="w-full" size="lg" onClick={submit} disabled={amount <= 0}>
                  تسجيل الدفعة
                </Button>
              </div>
            </div>
